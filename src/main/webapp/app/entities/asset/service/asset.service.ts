...
export class AssetService {
  ...
  update(asset: IAsset): Observable<EntityResponseType> {
    return this.http.put<IAsset>(`${this.resourceUrl}/${getAssetIdentifier(asset) as number}`, asset, { observe: 'response' });
  }

  partialUpdate(asset: IAsset): Observable<EntityResponseType> {
    return this.http.patch<IAsset>(`${this.resourceUrl}/${getAssetIdentifier(asset) as number}`, asset, { observe: 'response' });
  }
